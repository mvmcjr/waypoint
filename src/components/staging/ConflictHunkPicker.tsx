 ...
 *
 * Every mode requires explicit confirmation (`approved: true`) before the file
 * is written, so the user always sees the result preview first.
 */
type HunkResolution =
  | { mode: "ours" | "theirs" | "both"; approved: boolean }
...
/**
 * Build the resolved file content from segments + resolution map.
 *
 * Context parts are always included (even blank-line-only segments), which
 * preserves single blank lines that sit between conflict blocks.
 * Empty conflict resolutions (the hunk is intentionally deleted) are omitted
 * rather than contributing an empty string, which would otherwise insert a
 * spurious blank line via the "\n" separator.
 */
function buildResolved(segments: Segment[], resolutions: Map<number, HunkResolution>): string {
  const parts: string[] = [];
  for (const seg of segments) {
    if (seg.kind === "context") {
      // Always push context — including the empty-string case ("" from a single
      // blank line). When joined with "\n", adjacent empty strings produce the
      // double-"\n" that represents the blank line in the original file.
      parts.push(seg.lines.join("\n"));
    } else {
      const lines = getResolvedLines(seg, resolutions.get(seg.idx));
      // Only push when there are actual resolved lines.  An empty resolution
      // (hunk deleted) contributes nothing — no extra "\n" separator.
      if (lines !== null && lines.length > 0) parts.push(lines.join("\n"));
    }
  }
  return parts.join("\n");
}

/** Set whole-hunk mode, clearing any custom line selection. Starts unapproved. */
function withModeSet(
  prev: Map<number, HunkResolution>,
  idx: number,
  mode: "ours" | "theirs" | "both",
): Map<number, HunkResolution> {
  return new Map(prev).set(idx, { mode, approved: false });
}
...
function ChoiceButton({
  ...
  const styles: Record<string, string> = {
    blue:   active ? "bg-blue-500/30 text-blue-200 border-blue-400/60"       : "text-blue-300/75 border-blue-400/30 hover:text-blue-200 hover:bg-blue-500/20",
    purple: active ? "bg-purple-500/30 text-purple-200 border-purple-400/60" : "text-purple-300/75 border-purple-400/30 hover:text-purple-200 hover:bg-purple-500/20",
    teal:   active ? "bg-teal-500/25 text-teal-200 border-teal-400/50"       : "text-muted-foreground/60 border-border/50 hover:text-teal-200 hover:bg-teal-500/15",
  };
  return (
...
/** A single toggleable code line. ○ = excluded, ● = included. */
function ToggleableLine({
  ...
  // Selected row: coloured background + vibrant text.
  // Excluded row: no background change, explicit muted-but-readable colours.
  // We never dim the whole wrapper — that stacks opacities and makes text
  // nearly invisible in a dark theme.
  const wrapperBg  = included
    ? (tint === "blue" ? "bg-blue-500/20 hover:bg-blue-500/28" : "bg-purple-500/20 hover:bg-purple-500/28")
    : "hover:bg-white/[0.04]";
  const dotColor   = included
    ? (tint === "blue" ? "text-blue-400"    : "text-purple-400")
    : "text-muted-foreground/40";
  const numColor   = included ? "text-muted-foreground/50" : "text-muted-foreground/40";
  const textColor  = included
    ? (tint === "blue" ? "text-blue-100" : "text-purple-100")
    : "text-foreground/55";
  ...
  return (
    <button
      ...
      className={`w-full flex items-start text-left transition-colors ${wrapperBg}`}
    >
      {/* ● / ○ indicator */}
      <span className={`w-5 shrink-0 text-center text-[9px] pt-[3px] select-none ${dotColor}`}>
        {included ? "●" : "○"}
      </span>
      {/* Line number */}
      <span className={`w-7 shrink-0 text-right pr-2 font-mono text-[10px] leading-[1.7] select-none border-r border-white/5 ${numColor}`}>
        {lineNo + 1}
      </span>
      {/* Content */}
      <span className={`pl-2 font-mono text-[11px] leading-[1.7] whitespace-pre overflow-hidden min-w-0 flex-1 ${textColor}`}>
        {text || "​"}
      </span>
    </button>
  );
...
function ResultLine({ text, lineNo }: { text: string; lineNo: number }) {
  return (
    <div className="flex items-start bg-teal-500/[0.07]">
      <span className="w-7 shrink-0 text-right pr-2 font-mono text-[10px] leading-[1.7] text-muted-foreground/40 select-none border-r border-white/5">
        {lineNo + 1}
      </span>
      <span className="pl-2 font-mono text-[11px] leading-[1.7] whitespace-pre overflow-hidden text-teal-100/90">
        {text || "​"}
      </span>
    </div>
  );
...
function StackedHunk({
  seg, num, total, resolution, onSetMode, onToggleLine, onApprove,
}: {
  ...
  const isCustomEmpty  = resolution?.mode === "custom"
    && resolution.oursLines.size === 0
    && resolution.theirsLines.size === 0;
    ...
    <div className="border border-border/50 rounded-lg overflow-hidden mb-4">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-white/[0.03] border-b border-border/40">
        ...
        {isFinalized(resolution) && (
          <span className="text-[10px] text-green-400/90 font-medium">✓ resolved</span>
        )}
      </div>
      ...
      <div className="border-l-[3px] border-blue-400/60">
        <div className="flex items-center justify-between px-3 py-1 border-b border-border/20 bg-blue-500/[0.04]">
          <span className="text-[10px] font-mono text-blue-300/85 truncate min-w-0 mr-2">
            ← {seg.oursLabel || "HEAD"}
          </span>
        ...
        {seg.ours.length > 0
          ...
          : <div className="px-4 py-2 text-[10px] italic text-muted-foreground/50">(empty — this side deletes these lines)</div>
        }
      </div>
      ...
      <div className="border-l-[3px] border-purple-400/60">
        <div className="flex items-center justify-between px-3 py-1 border-b border-border/20 bg-purple-500/[0.04]">
          <span className="text-[10px] font-mono text-purple-300/85 truncate min-w-0 mr-2">
      ...
      {/* Per-hunk result — shown as soon as any choice is made */}
      {resolvedLines !== null && (
        <div className="border-t border-border/40 bg-teal-950/25">
          <div className="px-3 py-1 text-[10px] uppercase tracking-widest text-teal-400/70 font-semibold border-b border-border/20 flex justify-between items-center">
            <span>Result</span>
            {resolution && (
              resolution.approved ? (
                ...
                <button
                  ...
                  title={isCustomEmpty ? "Select at least one line to accept" : undefined}
                  className={`text-[9px] px-2 py-0.5 rounded bg-teal-500/20 text-teal-200 border border-teal-500/35 hover:bg-teal-500/30 transition-colors normal-case font-medium ${
                    isCustomEmpty ? "opacity-40 cursor-not-allowed" : "cursor-pointer"
                  }`}
                >
                  Accept
                </button>
          ...
          {resolvedLines.length > 0
            ...
            : <div className="px-4 py-2 text-[10px] italic text-muted-foreground/50">(empty — section will be deleted)</div>
          }
        </div>
      )}
    </div>
      ...
      <div className="grid grid-cols-3 divide-x divide-border/40">
        ...
        <div className="border-l-[3px] border-blue-400/60 min-w-0 overflow-hidden">
          <div className="flex items-center justify-between px-2 py-1 border-b border-border/20 bg-blue-500/[0.04]">
            <span className="text-[10px] font-mono text-blue-300/85 truncate min-w-0 mr-1">
              ← {seg.oursLabel || "HEAD"}
            </span>
            <ChoiceButton active={resolution?.mode === "ours"} tint="blue" label="Ours" onClick={() => onSetMode("ours")} />
          </div>
          {seg.ours.length > 0
            ...
            : <div className="px-3 py-3 text-[10px] italic text-muted-foreground/50">(empty)</div>
          }
        </div>
        ...
        <div className="border-l-[3px] border-purple-400/60 min-w-0 overflow-hidden">
          <div className="flex items-center justify-between px-2 py-1 border-b border-border/20 bg-purple-500/[0.04]">
            <span className="text-[10px] font-mono text-purple-300/85 truncate min-w-0 mr-1">
              → {seg.theirsLabel || "theirs"}
            </span>
        ...
        <div className="border-l-[3px] border-teal-400/40 min-w-0 overflow-hidden">
          <div className="flex items-center px-2 py-1 border-b border-border/20 bg-teal-500/[0.04] justify-between">
            <span className="text-[10px] text-teal-300/80 uppercase tracking-widest font-semibold">
              Result
            </span>
            {resolution && (
              resolution.approved ? (
                ...
                <button
                  ...
                  disabled={isCustomEmpty}
                  title={isCustomEmpty ? "Select at least one line to accept" : undefined}
                  className={`text-[9px] px-2 py-0.5 rounded bg-teal-500/20 text-teal-200 border border-teal-500/35 hover:bg-teal-500/30 transition-colors font-medium ${
                    isCustomEmpty ? "opacity-40 cursor-not-allowed" : "cursor-pointer"
                  }`}
                >
                  Accept
          ...
          {resolvedLines === null ? (
            <div className="px-3 py-3 text-[10px] italic text-muted-foreground/50">
...
function ResultFilePreview({
  ...
  type Row =
    | { type: "line"; lineNo: number | null; text: string; resolved: boolean }
  ...
  const rows = useMemo<Row[]>(() => {
    const result: Row[] = [];
    let lineNo = 1;
    // Once we've passed an unresolved hunk we don't know how many lines it will
    // contribute, so line numbers become uncertain — show "?" instead.
    let uncertain = false;

    for (const seg of segments) {
      if (seg.kind === "context") {
        for (const line of seg.lines) {
          result.push({ type: "line", lineNo: uncertain ? null : lineNo++, text: line, resolved: false });
        }
      } else {
        ...
        if (lines === null) {
          ...
          for (const line of lines) {
            result.push({ type: "line", lineNo: uncertain ? null : lineNo++, text: line, resolved: true });
          }
        }
      }
    }
    ...
    <div className="flex-1 overflow-y-auto min-h-0 font-mono text-[11px] leading-[1.7]">
      {rows.map((row, i) => {
        if (row.type === "unresolved") {
          return (
            <div key={i} className="flex items-center gap-2 px-4 py-1.5 bg-orange-500/10 border-y border-orange-400/20 text-[10px] text-orange-400/85">
        ...
        if (row.type === "empty-resolved") {
          return (
            <div key={i} className="px-4 py-0.5 text-[10px] italic text-muted-foreground/50 bg-teal-500/5 border-y border-teal-400/10">
        ...
        return (
          <div key={i} className={`flex items-start ${row.resolved ? "bg-teal-500/[0.07]" : ""}`}>
            <span className="w-10 shrink-0 text-right pr-3 text-muted-foreground/40 select-none border-r border-border/20">
              {row.lineNo ?? "?"}
            </span>
            <span className={`pl-3 whitespace-pre overflow-hidden ${row.resolved ? "text-teal-100/90" : "text-foreground/90"}`}>
              {row.text || "​"}
            </span>
          </div>
        );
...
export function ConflictHunkPicker({ repoId, path, viewMode, onResolved }: Props) {
  ...
  //
  // `repoId` and `path` are intentionally omitted from deps — they're captured
  // at fire-time (correct, since they identify the file we're resolving).
  // `onResolved` is accessed through `onResolvedRef` so it's always fresh.
  // The `active` cleanup flag ensures state mutations are skipped if the
  // component unmounts (e.g. user switches files) while the IPC call is in-flight.
  useEffect(() => {
  ...
  const setMode = useCallback((idx: number, mode: "ours" | "theirs" | "both") => {
    setResolutions((prev) => withModeSet(prev, idx, mode));
  }, []);

  const toggleLine = useCallback((seg: ConflictSeg, side: "ours" | "theirs", lineIdx: number) => {
    setResolutions((prev) => withLineToggled(prev, seg, side, lineIdx));
  }, []);

  const approveHunk = useCallback((idx: number) => {
    setResolutions((prev) => {
      const cur = prev.get(idx);
      if (!cur || cur.approved) return prev;
      // For custom mode: require at least one line selected to prevent
      // accidentally approving an empty selection that would delete content.
      if (cur.mode === "custom" && cur.oursLines.size === 0 && cur.theirsLines.size === 0) return prev;
      return new Map(prev).set(idx, { ...cur, approved: true });
    });
  }, []);
    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="shrink-0 flex items-center gap-4 px-4 py-2 border-b border-border/40">
        ...
        <div className="flex-1 flex items-center gap-2">
          ...
          <span className="text-[10px] text-muted-foreground/65 shrink-0 tabular-nums">
